# Korean Paper Agent Console (KPAC) · v0.5.5
...
### AI 작성팀 — 워크스페이스 편집 (v0.5.5, Claude 백엔드 기본)

Claude 백엔드에서는 채팅 지시가 **워크스페이스 편집**으로 처리됩니다: 에이전트(Claude Code)가 **프로젝트 폴더(src)를 작업 폴더로 직접 탐색(Grep/Glob/Read)·수정(Edit/Write)** 합니다. 기존 "전문을 프롬프트에 넣고 전문을 돌려받는" 방식과 달리,
...
Codex 백엔드(또는 워크스페이스 실패 시)는 아래 멀티에이전트 파이프라인으로 동작합니다.
...
─────────────────────────────────────────────────────────────
  ① writeOrchestrator   단계 계획(steps) — writing/figure/citation/evidence/research
   ...
   본문·그림/표 (멀티에이전트)          싱글/읽기전용
                                        · writeCitation : 빈 \cite{} 를 .bib 근거로 채움
