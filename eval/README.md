...
### ① QA 정확도 — [`results/summary.md`](results/summary.md)

| 시스템 | accuracy (150문항) | 평균 응답 | 비고 |
|---|---|---|---|
| agent-evidence | **90.7%** (136/150) | 13.5s / 1,228자 | 근거 발췌 + 위치 제시 포함 |
| single-llm | 86.7% (130/150) | 7.1s / 445자 | |

두 시스템의 판정이 갈린 문항은 12개(agent만 정답 9, single만 정답 3)로, 이 차이는
**통계적으로 유의하지 않다**(McNemar p≈0.15). 따라서 정확도 우위를 주장하지 않으며,
에이전트 구조의 가치는 아래 ②·③의 **검증 가능한 근거 제시와 할루시네이션 억제**에 있다.

### ② 할루시네이션 4축 — [`results/hallucination_summary.md`](results/hallucination_summary.md)

| 축 | 방법 | agent-evidence | single-llm |
|---|---|---|---|
| 인용 실재성 | 따옴표 인용을 원문과 문자열 대조 (LLM-free) | 위조 포함 run 13.3% → **가드 도입 후 0%** (아래 ③) | 5.3% |
...
- **표본**: 정확도 차이 4%p는 150문항으로는 유의하지 않음 — 우월 주장 대신 "동등 정확도 + 인용 신뢰성"으로 서술.
